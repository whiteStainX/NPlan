...
- [x] **Seeding System:** `DataSeeder` successfully loads 40+ exercises with detailed attributes from embedded JSON. Seeding now maps `SlotType` to its `rawValue` string and uses a helper to insert exercises + secondary muscles cleanly.
...
- [x] **App Container Hardening:** `NPlanApp` now retries `ModelContainer` creation after purging the local store if migration fails (dev-only safety valve to unblock schema changes).
...
**Operational Note (dev only):**
- Store purge fallback will wipe on-device data if migration fails. Acceptable for iteration; replace with real migration before shipping.
